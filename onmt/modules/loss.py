...
class CrossEntropyLossBase(_Loss):
    ...
    def _compute_loss(self, logits, targets, tgt_lang=None):
        ...
        eps_i = smoothing_value if self.training else 0.0
...
class NMTLossFunc(CrossEntropyLossBase):
    ...
    def forward(self, model_outputs, targets, model=None, vocab_mask=None, **kwargs):
        ...
        loss, loss_data = self._compute_loss(logits, targets, tgt_lang=lang)

        total_loss = loss

        if mirror:
            reverse_loss, rev_loss_data = self._compute_loss(reverse_logits, reverse_targets, tgt_lang=lang)
...
#
#     def __init__(self, vocab_sizes, model_size):
#
#         super(MultiCTC, self).__init__()
#
#         self.ctcs = nn.ModuleDict()
